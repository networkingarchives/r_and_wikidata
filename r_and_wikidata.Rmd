---
title: "Wikidata and Correspondence Archives"
...
This is an R Notebook: a document containing code and text. A live version can be run through [MyBinder](https://mybinder.org/v2/gh/networkingarchives/r_and_wikidata/master?urlpath=rstudio), which will spin up a copy of R-studio for you to use, and the source code is available on [GitHub](https://github.com/networkingarchives/r_and_wikidata).

More info [below](#info)
...
Wikidata can be queried using a language called [SPARQL](https://www.w3.org/TR/rdf-sparql-query/). It allows you to query the triples: essentially, you fill in two parts of the triple and the missing part is returned. These statements can be linked together to construct complicated queries.
...
That gravitational pull towards London is striking. Again, these queries can be built up, and one piece of data connected to another. A list of all those with parents born in different country to their birth place would be an interesting starting-point for understanding a second-generation diaspora network.

### Wikidata and EMLO

Another benefit of Wikidata is its links to other authority files, including an *Early Modern Letters Online* person ID (P1802). About 8,300 Wikidata entities have EMLO IDs listed - a full list can be downloaded with the following query:

```{r}
emlo_ids = query_wikidata("SELECT DISTINCT  ?item ?itemLabel WHERE
{
?item wdt:P1802 ?value.
  
  SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'.}
  
  }")

emlo_ids %>% head(10) %>% select(itemLabel)
```

EMLO already has good basic, structured biographical data, and Wikidata isn't going to add much there. But the knowledge graph contains other, more detailed structured information (such as membership lists), and combining queries allows us to filter on very specific pieces of information using the power of linked data.
...
```{r message=FALSE, warning=FALSE, cache=TRUE}
rs_q = query_wikidata ("SELECT DISTINCT  ?itemLabel ?child ?childLabel ?article ?valueLabel ?dob WHERE
{
...
spo_ids = rbind(spo_mapped_people %>% 
                  select(id = X1, wikilink  = X5), spo_added_people %>% select(id = X1, wikilink = `Linked data (if any)`)) %>% 
                  filter(wikilink %in% jamesdesc$article) %>% 
  filter(!is.na(wikilink))

james_network = spo_raw %>% 
  filter(X1 %in% spo_ids$id & X2 %in% spo_ids$id) %>% 
  distinct(X1, X2) %>% graph_from_data_frame() %>% 
  as_tbl_graph() %>%
...
```{r message=FALSE, warning=FALSE}
 ...
 james_network %>% 
  ...
  theme_void() + labs(title = "Letter Network of Descendants of James I/VI found in\nState Papers Online", subtitle = "Generated using Wikidata Query Service and SPO metadata") + theme(plot.title = element_text(face = 'bold', size = 16))
```
...
Hotson, Howard, and Thomas Wallnig, eds. 2019. *Reassembling the Republic of Letters in the Digital Age*. Göttingen University Press. https://doi.org/10.17875/gup2019-1146 - has discussion of linked data, particularly chapters I.3 and III.1 on the use of RDF in letter metadata.  
